	...
	>
		...
		>
			...
			<div class="form-section flex flex-col gap-4">
				<!-- <VInput
					:value="editRecipe.recipe.title"
					:title="t('recipeTitlePlaceholder')"
					:error="!!errors?.title"
					:error-message="errors?.title?.message"
					:max-length="100"
					@update:model-value="handleUpdateRecipe('title')"
					@focusout="validateField('title')"
				/>
				<VInput
					:value="editRecipe.recipe.description"
					:title="t('recipeDescriptionPlaceholder')"
					textarea
					:error="!!errors?.description"
					:error-message="errors?.description?.message"
					:max-length="500"
					@update:model-value="handleUpdateRecipe('description')"
					@focusout="validateField('description')"
				/> -->
				<VInput
					:model-value="editRecipe.recipe.title"
					:title="t('recipeTitlePlaceholder')"
					:max-length="100"
					name="title"
					@update:model-value="(value: string) => handleUpdateRecipe('title')(value)"
				/>
				<VInput
					:model-value="editRecipe.recipe.description"
					:title="t('recipeDescriptionPlaceholder')"
					name="description"
					textarea
					:max-length="500"
					@update:model-value="(value: string) => handleUpdateRecipe('description')(value)"
				/>
			</div>
...
import localization from './BasicInfo.localization.json'
import { editRecipeStore } from '@/features/Recipe/editRecipe'
import { useRoute } from 'vue-router'
import { VButton } from '@/shared/components/Button'

const editRecipe = editRecipeStore()

const route = useRoute()

const { t } = useTranslation(localization)

const handleUpdateRecipe = (field: string) => (value: string) => {
